...
def get_features_graph_model(n_features, n_classes, n_nodes):
    ...
    x1 = emb(in_2)
    x1 = Flatten()(x1)
    d1 = Dense(10, activation="relu", kernel_regularizer=l2(0.0005))
    x2 = d1(in_1)

    x = Concatenate()([x1, x2])

    d2 = Dense(n_classes, activation="softmax", kernel_regularizer=l1(0.05))
    x_out = d2(x)
    ...
    model2.compile(loss="mae", optimizer=Adam(lr))
...
for _ in range(1000):
    ...
    model2.fit_generator(gen_2(neigh_idxes= neigh_idxes, n_classes=int(max(Y_train) + 1),
                               node_int_features_mapping=node_int_features_mapping), steps_per_epoch=100, epochs=1,
                         verbose=2)
    print("model1")

    model1.fit([X_train, X_G_train], Y_train, validation_data=([X_val, X_G_val], Y_val),
               nb_epoch=5, verbose=2, callbacks=[early])
